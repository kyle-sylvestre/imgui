...
static uint32_t g_BackgroundRGB = 0;
...
void ImGui_ImplGDI_RenderDrawData(ImDrawData* draw_data)
{
    ...
    {
        // clear the draw context to background color
        for (size_t r = 0; r < fb_height; r++)
            for (size_t c = 0; c < fb_width; c++)
                memcpy(g_PixelBuffer + (r * fb_width) + c, &g_BackgroundRGB, sizeof(g_BackgroundRGB));
    ...
    imgui_sw::paint_imgui(g_PixelBuffer, fb_width, fb_height, SwOptions{});
...
void ImGui_ImplGDI_SetBackgroundColor(ImVec4* BackgroundColor)
{
    ...
    g_BackgroundRGB = RGB(BackgroundColor->x * 256.0,
                          BackgroundColor->y * 256.0,
                          BackgroundColor->z * 256.0);

    // endian swap to get ARGB
    ///g_BackgroundRGB =   ((g_BackgroundRGB >>24)&0xff) | // move byte 3 to byte 0
    ///                    ((g_BackgroundRGB <<8)&0xff0000) | // move byte 1 to byte 2
    ///                    ((g_BackgroundRGB >>8)&0xff00) | // move byte 2 to byte 1
    ///                    ((g_BackgroundRGB <<24)&0xff000000); // byte 0 to byte 3
