...
void ImGui_ImplGDI_RenderDrawData(ImDrawData* draw_data)
{
    ...
    if (old_fb_width != fb_width || old_fb_height != fb_height)
    {
        ...
        // Get the handle of the current window.
        //ImGuiIO& io = ImGui::GetIO();
        //HWND hWnd = reinterpret_cast<HWND>(io.ImeWindowHandle); OLD
        //HWND hWnd = reinterpret_cast<HWND>(ImGui::GetMainViewport()->PlatformHandleRaw);
...
void ImGui_ImplGDI_SetBackgroundColor(ImVec4* BackgroundColor)
{
    ...
    // TODO
    //g_BackgroundColor = RGB(BackgroundColor->x * 255.0,
